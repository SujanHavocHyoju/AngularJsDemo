...
app.controller("FooterController",footer);

app.value("AppNameService","My Contact App");
//or app.constant instead of a value for values that do not change
app.value("AppDataService",{
	"name":"Contact App",
	"author":"Sujan",
	"compay":"HAVOC",
	version:1
...
function prerareAppConfig(AppNameService){
	var value={};
...
app.factory("AppDataFactoryService",prerareAppConfig); //uses as a function
app.service("AppDataServiceSVC",AppConfig); //uses as a constructor
...
/*function header(AppNameService, AppDataFactoryService){
	this.appTitle=AppNameService;
	console.log(AppDataFactoryService);
} */
function header(AppDataServiceSVC){
	this.appTitle=AppDataServiceSVC.name;
	console.log(AppDataServiceSVC.name);
}

function footer(AppDataService, loggingSvc){
	this.appTitle=AppDataService.name + " Version:" +AppDataService.version ;
	loggingSvc();
}